...
class RTFFormat <  Format
    ...
    LIST_ITEM.font_size = FONT_SIZE
    LIST_ITEM.bold = true
    LIST_DESCRIPTION.font_size = FONT_SIZE
    SUPPLEMENTAL.left_indent = QUARTER_INCH
    SUPPLEMENTAL_TEXT.font_size = FONT_SIZE
    SUPPLEMENTAL_TEXT.italic = true
    H1.font = Font.new(Font::SWISS,"Helvetica")
    H1.font_size = 38
    H1.bold = true
    H2.font = H1.font
    H2.font_size = H1.font_size - 4
    H2.bold = true
    H3.font_size = H2.font_size - 10 
    H3.bold = true
    H4.font_size = H3.font_size - 2

    TEN_POINT = CharacterStyle.new
    TEN_POINT.font_size = FONT_SIZE
    ...
    def initialize(resume)
        ...
        @rtf.footer=footer
        @rtf.footer.apply(footer_style) { |n| n << "Resume of #{resume.core.contact_info.name}, generated by http://github.com/davetron5000/daves-resume" }
    ...
    def output_skillset
        ...
        rtf.table(1,2,INCH,5 * INCH) do |table|
            if !novice_skills.empty?
                table[0][0].apply(LIST_DESCRIPTION) { |n| n.bold() << "Exposure to:" }
